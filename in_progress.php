    ...
    <!--Navbar header-->
      ...
      <div class="collapse navbar-collapse" id="myNavbar">
        ...
        <?}?>
          ...
          <li class="active"><a href="in_progress.php">In Progress</a></li>
...
<center><h1>Standards In Progress</h1><hr></center>
<?
$table = '
<table class="table table-bordered table-striped" id="in_progress_table" align="center">
  <thead>
    <tr>
      <th>Assigned To</th>
      <th>Date Assigned</th>
  ...
  <tbody>';
    ...
    while($row = $db->fetch($sql)){
      $table .= "<tr>";
        ...
        if ($today > date("Y/m/d", strtotime($row['EXPECTED_COMPLETION'])) && $row['EXPECTED_COMPLETION'] != '' ){
        ...
        if (in_array($person, $readWrite_id)||in_array($person, $admin_id)) {
          ...
          $table .= "<td><button class='btn btn-success btn-sm' id='complete_btn' data-id='".$row['ID']."'><span class='glyphicon glyphicon-ok'></span></button></td>";
        }
        if ($row['UPLOAD'] != null)
        {
      ...
      $(document).on('click', '#complete_btn', function(e){
        ...
        $.ajax({
          ...
          success:function(data){
            alert('Successfully completed assignment');
            location.reload();
          }
        })};
      });
