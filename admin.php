<?php
  ...
  $sql = "SELECT * FROM OEP_DASHBOARD_ADMINS WHERE PERMISSIONS = 'ADMIN'";
  $sql_requested = "SELECT * FROM OEP_DASHBOARD_ADMINS WHERE PERMISSIONS = 'REQUESTED'";
?>
...
<div class="container">
<!-- <button id="log" type="button" onclick="log_function()">LOG</button>  --> 
</div>
  ...
  <script src="https://cdn.datatables.net/1.10.12/js/dataTables.bootstrap.min.js"></script>
   ...
   <script>
    $(document).ready(function(){
      ...
      $(document).on('click', '.approve_access_btn', function(){
        var user_id = this.getAttribute('data-userid');

        $.ajax({
          type:"POST",
          url:"scripts/approve_access.php",
          data: {
            user_id: user_id
          },
          success:function(data){
            alert('User Access Approved');
            location.reload();
          }
        });
      });

       $(document).on('click', '.deny_access_btn', function(){
        var user_id = this.getAttribute('data-userid');

        $.ajax({
          type:"POST",
          url:"scripts/deny_access.php",
          data: {
            user_id: user_id
          },
          success:function(data){
